...
class LabelsApi {
  ...
  static Future<http.Response> addLabel(String name, String date, String authToken) async {
    return await http.post(Uri.parse('http://localhost:9999/api/v1/label'), body: {'name': name, 'date': date}, headers: {'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': 'Bearer $authToken'});
  }

  static Future<http.Response> deleteLabel(String label) async {
    return await http.delete(Uri.parse('http://localhost:9999/api/v1/labels'), body: label, headers: {'Content-Type': 'application/json'});
  }
